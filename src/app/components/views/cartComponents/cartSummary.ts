...
export class CartSummaryContent {
  constructor(node: HTMLElement, arrayData: number[]) {
    ...
    const countProduct = new Element(blockProductSummary.elem, 'p', 'summary-product__count', `${arrayData[0]}`)
    ...
    const countTotal = new Element(blockTotalSummary.elem, 'p', 'summary-total__count', `${arrayData[1]}`)
