...
export class CartHeaderContent {
  ...
  constructor(
    ...
    const titleContentiIgnor = new Element(headerCart.elem, HTMLTag.H2, 'cart-header__title', 'Products in Cart');
    const blockContent = new Element(headerCart.elem, HTMLTag.DIV, 'block-content');
    const blockItemProduct = new Element(blockContent.elem, HTMLTag.DIV, 'block-item');
    const itemProductIgnor = new Element(blockItemProduct.elem, HTMLTag.P, 'block-item__item', 'LIMIT:');
    ...
    const blockPagination = new Element(blockContent.elem, HTMLTag.DIV, 'block-pagination');
    const pagePaginationIgnor = new Element(blockPagination.elem, HTMLTag.P, 'block-pagination__page', 'PAGE:');
    const blockAmount = new Element(blockPagination.elem, HTMLTag.DIV, 'block-pagination__amount');
    const pageBtnLeft = new Element(blockAmount.elem, HTMLTag.BUTTON, 'block-pagination__btn', '<');
    this.pageNumber = new Element(blockAmount.elem, HTMLTag.P, 'block-pagination__number', `${this.count}`);
    const pageBtnRight = new Element(blockAmount.elem, HTMLTag.BUTTON, 'block-pagination__btn', '>');
